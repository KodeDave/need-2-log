...
{
    /// <summary>
    /// Fornisce 3 metodi static che permettono di calcolare un hash, confermarlo e generare randomicamente una password
    ...
    public class Cript
    {
        ...
        public static string ComputeHash(string plainText, byte[] salt)
        {
            ...
            if (!String.IsNullOrEmpty(plainText))
            {
                ...
                sha.Dispose();
                result = new byte[hashValue.Length + SaltBytes.Length];
                for (int i = 0; i < hashValue.Length; i++)
                    result[i] = hashValue[i];
                for (int i = 0; i < SaltBytes.Length; i++)
                    result[hashValue.Length + i] = SaltBytes[i];
            }
            return (String.IsNullOrEmpty(plainText))?"":Convert.ToBase64String(result);
        }
        /// <summary>
