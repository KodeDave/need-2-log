...
namespace N2L_Need_2_Log.gui
{
    ...
    public partial class FormCreate : Form
    {
        ...
        private void FormCreate_Load(object sender, EventArgs e)
        {
            try
            {
                this.database = DBConnector.Connect;
                ...
                if(!newRecord)
                {
                    ...
                    using (DbDataReader data = database.GetRecordInfo(item))
                    {
                        ...
                        this.pictureBox.Image = Image.FromFile("..\\..\\..\\Core\\Resources\\Pers\\icon\\" + itemRec.GetIcon());
        ...
        private void buttonOk_Click(object sender, EventArgs e)
        {
            if (newRecord)
            {
                try
                {
                    string entryName = database.CheckEntryName(this.textBoxName.Text);
            ...
            else
            {
                try
                {
                    if (this.textBoxName.Text != record.Name)
                    {
                        string entryName = database.CheckEntryName(this.textBoxName.Text);
                        this.textBoxName.Text = entryName;
        ...
        private void comboBoxAccountType_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            this.pictureBox.Image = Image.FromFile("..\\..\\..\\Core\\Resources\\Pers\\icon\\" + item.GetIcon());
            this.pictureBox.MaximumSize = new Size(128, 128);
